/**
 * Chromatist JavaScript library v0.1.0
 *
 * Copyright 2011, Jacob Rus
 */
(function() {
/* chromatist.coffee */
var chromatist, root, _;
...
/* underscore_mixins.coffee */
(function() {
  var mod;

  mod = chromatist.mathutils.mod;
...
(function() {
  ...
  Matrix3 = (function() {
    ...
    Matrix3.prototype._matrix_multiply = function(other) {
      var M00, M01, M02, M10, M11, M12, M20, M21, M22, N00, N01, N02, N10, N11, N12, N20, N21, N22, _ref, _ref1;
      _ref = this.matrix, M00 = _ref[0], M01 = _ref[1], M02 = _ref[2], M10 = _ref[3], M11 = _ref[4], M12 = _ref[5], M20 = _ref[6], M21 = _ref[7], M22 = _ref[8];
      _ref1 = other.matrix, N00 = _ref1[0], N01 = _ref1[1], N02 = _ref1[2], N10 = _ref1[3], N11 = _ref1[4], N12 = _ref1[5], N20 = _ref1[6], N21 = _ref1[7], N22 = _ref1[8];
    ...
    Matrix3.prototype.dot = function(other) {
      var _ref, _ref1;
      if (((_ref = other.shape) != null ? _ref[0] : void 0) === 3 && ((_ref1 = other.shape) != null ? _ref1[1] : void 0) === 3 && (other.matrix != null)) {
...
/* cie.coffee */
(function() {
  var cie, isArray, reduce, standard_whitepoints, sum;

  cie = chromatist.cie = {};

  reduce = _.reduce, isArray = _.isArray;

  sum = function(list) {
    var plus;
    plus = function(total, x) {
      return total + x;
    };
    return reduce(list, plus, 0);
  };
  ...
  cie.normalize_chromaticity = function(c) {
    var c_x, c_y, x, _ref;
    if (!(isArray(c) && ((_ref = c.length) === 2 || _ref === 3))) {
...
/* cat.coffee */
(function() {
  var Matrix3, cat, normalize_whitepoint, print, transforms;
  ...
  Matrix3 = chromatist.matrix3.Matrix3;

  normalize_whitepoint = chromatist.cie.normalize_whitepoint;

  transforms = {
    'von kries': [.38971, .68898, -.07868, -.22981, 1.18340, .04641, .00000, .00000, 1.00000],
    'linear bradford': [.8951, .2664, -.1614, -.7502, 1.7135, .0367, .0389, -.0686, 1.0296],
    cmccat2000: [.7982, .3389, -.1371, -.5918, 1.5512, .0406, .0008, .0239, .9753],
    cat02: [.7328, .4296, -.1624, -.7036, 1.6975, .0061, .0030, .0136, .9834]
  };

  cat.Converter = function(from_white, to_white, transform) {
    var forward_matrix, from_L, from_M, from_S, gain_matrix, to_L, to_M, to_S, _ref, _ref1;
    if (transform == null) {
      transform = 'cat02';
    }
    ...
    _ref = transform.dot(from_white), from_L = _ref[0], from_M = _ref[1], from_S = _ref[2];
    _ref1 = transform.dot(to_white), to_L = _ref1[0], to_M = _ref1[1], to_S = _ref1[2];
    gain_matrix = new Matrix3([to_L / from_L, 0, 0, 0, to_M / from_M, 0, 0, 0, to_S / from_S]);
...
/* rgb.coffee */
(function() {
  var Matrix3, ProPhoto_gamma, RGB_matrix_from_primaries, RGB_spaces_parameters, every, isFunction, isNumber, isString, max, normalize_chromaticity, normalize_whitepoint, pow, rgb, round, sRGB_gamma, standard_whitepoints, _ref;

  rgb = chromatist.rgb = {};

  max = Math.max, pow = Math.pow, round = Math.round;

  _ref = chromatist.cie, standard_whitepoints = _ref.standard_whitepoints, normalize_chromaticity = _ref.normalize_chromaticity, normalize_whitepoint = _ref.normalize_whitepoint;

  Matrix3 = chromatist.matrix3.Matrix3;

  every = _.every, isString = _.isString, isNumber = _.isNumber, isFunction = _.isFunction;

  RGB_matrix_from_primaries = function(_arg) {
    var b, c_b, c_g, c_r, g, primaries, r, white, x, x_b, x_g, x_r, y_b, y_g, y_r, z_b, z_g, z_r, _ref1, _ref2;
    ...
    _ref1 = primaries.flat(), x_r = _ref1[0], x_g = _ref1[1], x_b = _ref1[2], y_r = _ref1[3], y_g = _ref1[4], y_b = _ref1[5], z_r = _ref1[6], z_g = _ref1[7], z_b = _ref1[8];
  ...
  rgb.Converter = function(params) {
    var decoding_gamma, encoding_gamma, from_XYZ, from_XYZ_linear, g, gamma_decode, gamma_encode, in_gamut, matrix, to_XYZ, to_XYZ_linear, _ref1;
    if (params == null) {
      params = 'sRGB';
    }
    if (isString(params)) {
    ...
    g = params.gamma;
    if (isNumber(g)) {
      _ref1 = [g, 1 / g], decoding_gamma = _ref1[0], encoding_gamma = _ref1[1];
    ...
    } else if (g.length === 2 && isFunction(g[0]) && isFunction(g[1])) {
    ...
    from_XYZ = function(XYZ) {
      var component, _i, _len, _ref2, _results;
      _ref2 = from_XYZ_linear(XYZ);
      _results = [];
  ...
  rgb.to_hex = function(_arg) {
    ...
    return '#' + (1 << 24 | round(0xff * R) << 16 | round(0xff * G) << 8 | round(0xff * B)).toString(16).slice(1);
  };
...
/* ciecam.coffee */
(function() {
  var Matrix3, abs, atan2, ciecam, cos, exp, floor, hue_comp_re, interpolate, mod, pow, round, sgn, sin, sqrt, standard_whitepoints, tau, unique_hues_H, unique_hues_e, unique_hues_h, unique_hues_s, _ref;
  ...
  _ref = chromatist.mathutils, sgn = _ref.sgn, mod = _ref.mod, interpolate = _ref.interpolate;

  Matrix3 = chromatist.matrix3.Matrix3;

  standard_whitepoints = chromatist.cie.standard_whitepoints;
  ...
  ciecam.Converter = function(params) {
    ...
    _ref1 = (function() {
      ...
      return _results;
    })(), D_R = _ref1[0], D_G = _ref1[1], D_B = _ref1[2];
    corresponding_colors = function(XYZ) {
      var B, G, R, _ref2;
      _ref2 = XYZ_to_CAT02(XYZ), R = _ref2[0], G = _ref2[1], B = _ref2[2];
    ...
    forward_model = function(XYZ) {
      var A, B_a, C, G_a, J, M, Q, RGB_a, RGB_c, R_a, a, b, e_t, h, h_rad, s, t, _ref2;
      RGB_c = corresponding_colors(XYZ);
      _ref2 = RGB_a = adapted_responses(RGB_c), R_a = _ref2[0], G_a = _ref2[1], B_a = _ref2[2];
    ...
    reverse_model = function(inputs) {
      ...
      if (C == null) {
        C = M != null ? M / pow(F_L, .25) : pow(s / 100, 2) * Q / pow(F_L, .25);
      }
      if (M == null) {
        M = C * pow(F_L, .25);
      }
  ...
  ciecam.inverse_hue_quad = function(H) {
    var amt, e_j, e_k, h, h_j, h_k, j, _ref1, _ref2;
    H = mod(H, 400);
    j = floor(H / 100);
    amt = H % 100;
    _ref1 = unique_hues_e.slice(j, (j + 1) + 1 || 9e9), e_j = _ref1[0], e_k = _ref1[1];
  ...
  ciecam.hue_comp = function(H) {
    var amt, j, s_j, s_k, _ref1;
    H = mod(round(H), 400);
    j = floor(H / 100);
    amt = H % 100;
    _ref1 = unique_hues_s.slice(j, (j + 1) + 1 || 9e9), s_j = _ref1[0], s_k = _ref1[1];
    if (amt === 0) {
      return "100" + s_j;
    } else {
  ...
  ciecam.parse_hue_comp = function(comp) {
    var amt_j, amt_k, j, k, s_j, s_k, s_uniq, whole_match, _ref1, _ref2, _ref3, _ref4, _ref5;
    _ref1 = comp.match(hue_comp_re), whole_match = _ref1[0], s_uniq = _ref1[1], amt_j = _ref1[2], s_j = _ref1[3], amt_k = _ref1[4], s_k = _ref1[5];
    if (whole_match == null) {
...
/* gamut.coffee */
(function() {
  var abs, codepoints, disp_CIECAM, gamut, interpolate, mod, polar, pow, rectangular, sRGB, tau, _ref;

  gamut = chromatist.gamut = {};

  pow = Math.pow, abs = Math.abs;

  _ref = chromatist.mathutils, mod = _ref.mod, polar = _ref.polar, rectangular = _ref.rectangular, interpolate = _ref.interpolate;
  ...
  gamut.bring_into_sRGB = function(XYZ, iterations) {
    var C, J, h, i, orig, prev_test, test_XYZ, this_test, _i, _ref1;
    if (iterations == null) {
      iterations = 30;
    }
    if (XYZ.J != null) {
      ...
      _ref1 = disp_CIECAM.forward_model(XYZ), J = _ref1.J, C = _ref1.C, h = _ref1.h;
    }
    orig = {
      J: J,
      C: C,
  ...
  gamut.Boundary = function(precision) {
    var circular_distance, decode_pt, distance, encode_pt, horiz_edges, horizontal, vert_edges, vertical, vertices, _ref1;
    if (precision == null) {
      precision = 32;
    }
    ...
    vertices = (function() {
      var V, add_vertex, i, j, n, p, s_curve, _i, _j, _k;
      V = {};
      p = precision;
      s_curve = function(comp) {
        return 2 * pow(comp, 3) - pow(comp, 5.5);
      };
      add_vertex = function(rgb) {
        var C, J, a_C, b_C, comp, h, point, xyz, _ref1, _ref2;
        xyz = sRGB.to_XYZ_linear((function() {
          var _i, _len, _results;
          _results = [];
          for (_i = 0, _len = rgb.length; _i < _len; _i++) {
            comp = rgb[_i];
            _results.push(s_curve(comp / p));
          }
          return _results;
        })());
        _ref1 = disp_CIECAM.forward_model(xyz), J = _ref1.J, C = _ref1.C, h = _ref1.h;
      ...
      for (i = _i = 0; 0 <= precision ? _i <= precision : _i >= precision; i = 0 <= precision ? ++_i : --_i) {
        for (j = _j = 0; 0 <= precision ? _j <= precision : _j >= precision; j = 0 <= precision ? ++_j : --_j) {
          for (n = _k = 0; _k < 3; n = ++_k) {
    ...
    _ref1 = (function() {
      var E_h, E_v, add_edge, add_edges, i, j, lower_vertices, n, p, upper_vertices, v, _i, _j, _k;
      E_h = {};
      E_v = {};
      p = precision;
      add_edge = function(edge_obj, rgb1, rgb2) {
        var C1, C2, J1, J2, h1, h2, v1, v2, _ref1, _ref2, _ref3;
        if (_.min(rgb1.concat(rgb2)) >= 0 && _.max(rgb1.concat(rgb2)) <= p) {
          _ref1 = [encode_pt(rgb1), encode_pt(rgb2)].sort(), v1 = _ref1[0], v2 = _ref1[1];
          _ref2 = vertices[v1], J1 = _ref2[0], C1 = _ref2[1], h1 = _ref2[2];
          _ref3 = vertices[v2], J2 = _ref3[0], C2 = _ref3[1], h2 = _ref3[2];
          return edge_obj[v1 + v2] = [v1, J1, h1, v2, J2, h2];
        }
      };
        ...
        for (j = _j = 0; 0 <= precision ? _j <= precision : _j >= precision; j = 0 <= precision ? ++_j : --_j) {
          ...
          for (n = _k = 0; _k < 3; n = ++_k) {
            add_edges((function() {
              ...
              _results = [];
              for (_l = 0, _len = upper_vertices.length; _l < _len; _l++) {
                v = upper_vertices[_l];
            ...
            add_edges((function() {
              var _l, _len, _results;
              _results = [];
              for (_l = 0, _len = lower_vertices.length; _l < _len; _l++) {
                v = lower_vertices[_l];
                _results.push(_(v).rotate(n));
              }
              return _results;
      ...
      return [E_h, E_v];
    })(), horiz_edges = _ref1[0], vert_edges = _ref1[1];
    distance = function(p_0, p_1, x) {
      return (x - p_0) / (p_1 - p_0);
    };
    circular_distance = function(a_0, a_1, max_a, x) {
      var shift_amount, y, _ref2, _ref3;
      _ref2 = [mod(a_0, max_a), mod(a_1, max_a)], a_0 = _ref2[0], a_1 = _ref2[1];
      ...
      _ref3 = (function() {
        ...
        return _results;
      })(), a_0 = _ref3[0], a_1 = _ref3[1], x = _ref3[2];
      return distance(a_0, a_1, x);
    };
    horizontal = function(J) {
      var C1, C2, Cx, J1, J2, a_C1, a_C2, a_Cx, b_C1, b_C2, b_Cx, dist, edge, h1, h2, hx, output, v1, v2;
      if (J == null) {
        J = 50;
      }
      output = (function() {
        var _ref2, _ref3, _ref4, _ref5, _results;
        _results = [];
        for (edge in horiz_edges) {
          _ref2 = horiz_edges[edge], v1 = _ref2[0], J1 = _ref2[1], h1 = _ref2[2], v2 = _ref2[3], J2 = _ref2[4], h2 = _ref2[5];
          dist = distance(J1, J2, J);
          if (!((0 <= dist && dist <= 1))) {
            continue;
          }
          _ref3 = vertices[v1], J1 = _ref3[0], C1 = _ref3[1], h1 = _ref3[2], a_C1 = _ref3[3], b_C1 = _ref3[4];
          _ref4 = vertices[v2], J2 = _ref4[0], C2 = _ref4[1], h2 = _ref4[2], a_C2 = _ref4[3], b_C2 = _ref4[4];
          a_Cx = interpolate(a_C1, a_C2, dist);
          b_Cx = interpolate(b_C1, b_C2, dist);
          _ref5 = polar([a_Cx, b_Cx]), Cx = _ref5[0], hx = _ref5[1];
          hx = mod(hx, tau);
          _results.push([a_Cx, b_Cx, hx]);
        }
        return _results;
      })();
    ...
    vertical = function(h) {
      var C1, C2, J1, J2, a_C1, a_C2, b_C1, b_C2, dist, edge, h1, h2, output, v1, v2;
      if (h == null) {
        h = 0;
      }
      h = tau / 360 * h;
      output = (function() {
        var _ref2, _ref3, _ref4, _results;
        _results = [];
        for (edge in vert_edges) {
          _ref2 = vert_edges[edge], v1 = _ref2[0], J1 = _ref2[1], h1 = _ref2[2], v2 = _ref2[3], J2 = _ref2[4], h2 = _ref2[5];
          ...
          _ref3 = vertices[v1], J1 = _ref3[0], C1 = _ref3[1], h1 = _ref3[2], a_C1 = _ref3[3], b_C1 = _ref3[4];
          _ref4 = vertices[v2], J2 = _ref4[0], C2 = _ref4[1], h2 = _ref4[2], a_C2 = _ref4[3], b_C2 = _ref4[4];
          _results.push([interpolate(J1, J2, dist), interpolate(C1, C2, dist)]);
        }
        return _results;
      })();
      output = output.concat([[0, 0], [100, 0]]);
      output.sort(function(a, b) {
        return a[0] - b[0];
      });
      return output;
